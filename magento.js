  ...
  this.client.methodCall('call', callArr, function(err) {
    ...
    if (err) {
      ...
      if (err.faultCode === 5 && !retried) {
        deb('/lib/magento-1.9:warning')('***** Detected session timeout at Magento XML-RPC level. Trying to re-login');

        self.login(function(err, sessionId) {
          if (err) {
            deb('/lib/magento-1.9:warning')('***** Session re-login unsuccessful.', err);
            callback(new MagentoError(err.faultString ? err.faultString : 'An error occurred while calling ' + method, err));
            return;
          }

          try {
            deb('/lib/magento-1.9:log')('***** Session re-login successful.');
            retryArgs.push(true);
            self.methodApply.apply(self, retryArgs);
          }
          catch (ex) {
            deb('/lib/magento-1.9:log')('***** Error trying to retry function call *****');
            deb('/lib/magento-1.9:log')('Message:   %s', ex.message);
            deb('/lib/magento-1.9:log')('Exception: %j', ex);
            deb('/lib/magento-1.9:log')('Args: %j', retryArgs);
            throw ex;
          }
        });
      } else {
